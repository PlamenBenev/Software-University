...
export function dashboardRequest() {
    const getOption = getOptions('get');
    const result = request(url + `/data/shoes?sortBy=_createdOn%20desc`, getOption)
...
export function createRequest(body) {
    const getOption = getOptions('post', body);
    const result = request(url + `/data/shoes`, getOption)
...
export function detailsRequest(id) {
    const getOption = getOptions('get');
    const result = request(url + `/data/shoes/` + id, getOption)
...
export function searchRequest(query) {
    const getOption = getOptions('get');
    const result = request(url + `/data/shoes?where=brand%20LIKE%20%22${query}%22`, getOption)
    return result;
}
